...
void jansion_uart_callback( uint8 port, uint8 event ){
...
void SimpleBLECentral_Init( uint8 task_id )
{
  ...
  NPI_InitTransport(jansion_uart_callback);
  NPI_WriteTransport("ble central", 12);
...
uint16 SimpleBLECentral_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  if ( events & START_DEVICE_EVT )
  {
    ...
    if (!simpleBLEScanning && simpleBLEState != BLE_STATE_CONNECTED && simpleBLEScanRes == 0)
    {
        ...
        GAPCentralRole_StartDiscovery( DEFAULT_DISCOVERY_MODE,
                                       DEFAULT_DISCOVERY_ACTIVE_SCAN,
                                       DEFAULT_DISCOVERY_WHITE_LIST );  
        LCD_WRITE_STRING( "scanning...\n", HAL_LCD_LINE_1 );
        
    }
    else{
    
        LCD_WRITE_STRING( "no scanning...", HAL_LCD_LINE_1 );
...
static void simpleBLECentral_HandleKeys( uint8 shift, uint8 keys )
{
  ...
  if(keys & HAL_KEY_SW_1){
     NPI_WriteTransport("key_1", 5);
    
  }
  if(keys & HAL_KEY_SW_2){
     NPI_WriteTransport("key_2", 5);
  ...
  switch ( pEvent->gap.opcode )
  {
    ...
    case GAP_DEVICE_DISCOVERY_EVENT:
    {
        ...
        if ( simpleBLEScanRes > 0 )
        {
            ...
            //connect the first device in ble dev list
            // Connect or disconnect
            if ( simpleBLEState == BLE_STATE_IDLE )
            {
                // if there is a scan result
